...
from .. import db
from ..models import Permission, User, Post, Comment, Notification, Like, Transaction, Collect, Want, Activity
...
def index_transaction():
    if request.method == 'GET':
        ...
        for item in li:
            ...
            item.important = li_num
        hot_activity = li.order_by(Activity.important.desc())
        return render_template('index.html', transactions=transactions, posts5=hot,
                               hot_activity=hot_activity, panel_id=2)
    else:
        inf = request.form["search"]
        return redirect(url_for('.query', content=inf))
...
def index_activity():
    if request.method == 'GET':
        ...
        for item in li:
            ...
            item.important = li_num
        hot_activity = li.order_by(Activity.important.desc())
        return render_template('index.html', activities=activities, posts5=hot,
                               hot_activity=hot_activity, panel_Id=3)
    else:
        inf = request.form["search"]
        return redirect(url_for('.query', content=inf))
...
def index_favorites():
    if request.method == 'GET':
        ...
        for item in li:
            ...
            item.important = li_num
        hot_activity = li.order_by(Activity.important.desc())
        return render_template('index.html', favorites=[_favorite.favorite_post for _favorite in current_user.favorite_post],
